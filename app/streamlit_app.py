...
def main() -> None:
    ...
    thresholds = json.loads(Path(args.thresholds).read_text()) if Path(args.thresholds).exists() else {"balanced": 0.74}
    ...
    model, extractor, classes, device = load_model(args.checkpoint)
    if not args.checkpoint:
        st.warning("No checkpoint supplied. The app is running with random weights for UI testing.")

    st.caption("Drag and drop satellite tile images into the boxes below, or click Browse files.")
    before_file = st.file_uploader(
        "Before tile",
        type=ACCEPTED_IMAGE_TYPES,
        help="Drag and drop one before image here, or click Browse files.",
    )
    after_file = st.file_uploader(
        "After tile",
        type=ACCEPTED_IMAGE_TYPES,
        help="Drag and drop one after image here, or click Browse files.",
    )
